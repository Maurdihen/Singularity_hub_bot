...
async def handle_start_command(message: types.Message):
    ...
    await bot.send_message(chat_id=message.chat.id,
                           text=f"Здравствуйте {message.from_user.username}, я бот Singularity Hub - meet, хотите приобрести бесплатные билеты на наши мероприятия?",
...
async def events_handler(call: types.CallbackQuery):
    ...
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                text='Вот доступные мероприятия:', reply_markup=inline_markup)
...
async def process_phone_number(message: types.Message, state: FSMContext):
    ...
    sent_message = await message.answer("Ваши данные записанны. Вы точно будете на мероприятии?", reply_markup=markup)
    global data1
    data1['message_id'] = sent_message.message_id
    await UserInfo.next()
...
@dp.callback_query_handler(lambda call: call.data in ['sure', 'maybe', 'unlikely'], state=UserInfo.last)
async def process_event_decision(call: types.CallbackQuery, state: FSMContext):
    message_id = data1.get('message_id', None)
    ...
    markup1.add(
        InlineKeyboardButton("Вернуться к мероприятиям", callback_data="events")
    )

    await bot.send_message(chat_id=call.message.chat.id, text="Ваши данные:\n" + user_info, reply_markup=markup1)
